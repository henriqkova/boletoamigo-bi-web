/**
 * Adapter: Boleto Amigo XLSX (EGestor)
 *
...
module.exports = {
  id: 'boletoamigo-xlsx',
  label: 'Boleto Amigo XLSX (EGestor)',
  required_env: [],
  ...
  validate(config) {
    ...
    const cx = config.fontes && config.fontes['boletoamigo_xlsx'];
    if (!cx) errors.push('config.fontes.boletoamigo_xlsx nao definido');
    else if (!cx.files || cx.files.length === 0) errors.push('config.fontes.boletoamigo_xlsx.files vazio');
  ...
  async pull(config, dataDir) {
    ...
    const cx = config.fontes['boletoamigo_xlsx'];
    const files = cx.files || [];

    console.log('=== Boleto Amigo XLSX pull ===');
    ...
    fs.writeFileSync(path.join(dataDir, 'empresa.json'), JSON.stringify({
      nome_fantasia: config.cliente?.nome || 'Boleto Amigo',
      fonte: 'boletoamigo-xlsx',
    }));
    ...
    console.log(`=== Boleto Amigo XLSX OK: ${uniqueCount} movimentos (${movimentos.length} com dual-regime) ===`);
    return { fetched: movimentos.length, summary: { adapter: 'boletoamigo-xlsx', records: movimentos.length } };
  },
};
